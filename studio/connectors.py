...
class WB:
    ...
    def _request(self, method, url, **kwargs):
        ...
        r = requests.request(method, url, headers=self.h, timeout=60, **kwargs)
        if not r.ok:
            text = (r.text or '')[:900]
    ...
    def warehouse_stocks(self, nm_ids=None, limit=100000):
        ...
        payload = {
            'nmIds': ids,
            'chrtIds': [],
            'limit': min(max(int(limit), 1), 250000),
            'offset': 0,
        }
        data = self._request(
            'POST', self.ANALYTICS + '/api/analytics/v1/stocks-report/wb-warehouses',
            json=payload,
        )
        ...
        for row in rows:
            ...
            total += qty
            if nm is not None:
                by_nm[str(nm)] = by_nm.get(str(nm), 0.0) + qty
    ...
    def sales(self, days=30):
        return self._request(
            'GET', self.STATISTICS + '/api/v1/supplier/sales',
            params={'dateFrom': self._date_from(days), 'flag': 0},
        )

    def orders(self, days=30):
        return self._request(
            'GET', self.STATISTICS + '/api/v1/supplier/orders',
            params={'dateFrom': self._date_from(days), 'flag': 0},
        )
    ...
    def sales_summary(rows):
        ...
        for r in rows if isinstance(rows, list) else []:
            try:
                amount = float(r.get('finishedPrice') or r.get('priceWithDisc') or 0)
            except Exception:
                amount = 0.0
            try:
                pay = float(r.get('forPay') or 0)
            except Exception:
                pay = 0.0
            ...
            payout += pay
        return {
            'operations': len(rows) if isinstance(rows, list) else 0,
            'units': units,
            'returns': returns,
            'gross': gross,
            'payout': payout,
    ...
    def hydrate_products(self, items):
        ...
        stocks = self.warehouse_stocks(ids).get('by_nm', {})
        out = []
        for p in items:
            q = dict(p)
            nm = str(q.get('external_id') or '')
            q['price'] = prices.get(nm, q.get('price', 0))
            q['stock'] = stocks.get(nm, q.get('stock', 0))
            out.append(q)
        return out
    ...
    def answer_review(self, feedback_id, text):
        ...
        self._request(
            'POST', self.FEEDBACKS + '/api/v1/feedbacks/answer',
...
class Ozon:
    ...
    def _post(self, path, payload=None):
        if not self.client_id or not self.key:
            raise APIError('Не указаны Ozon Client ID / API key')
        r = requests.post(self.BASE + path, headers=self.h, json=payload or {}, timeout=60)
        if not r.ok:
            raise APIError(f'Ozon {r.status_code}: {(r.text or "")[:900]}')
