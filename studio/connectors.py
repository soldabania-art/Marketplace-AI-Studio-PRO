...
class WB:
    ...
    def _pace(cls, url):
        ...
        if 'advert-api.wildberries.ru' in host and path.endswith('/adv/v3/fullstats'):
            min_gap = 1.10
        elif 'finance-api.wildberries.ru' in host:
            min_gap = 1.00
        else:
            min_gap = 0.15
        with cls._rate_lock:
            now = time.monotonic()
            wait = min_gap - (now - cls._last_request.get(host, 0.0))
            if wait > 0:
                time.sleep(wait)
    ...
    def products_all(self, max_cards=10000):
        ...
        while len(out) < max(1, int(max_cards)):
            ...
            updated = nxt.get('updatedAt'); nm_id = nxt.get('nmID')
            marker = (updated, nm_id)
            if not updated or not nm_id or marker in seen:
                break
            seen.add(marker)
        ...
        for start in range(0, len(ids), 1000):
            ...
            for item in (data.get('data', {}) or {}).get('listGoods', []) or []:
                ...
                for s in sizes:
                    ...
                    if val is None: val = s.get('price')
