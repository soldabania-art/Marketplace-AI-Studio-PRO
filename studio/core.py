...
PUBLIC_DEFAULTS={'openai_model':'gpt-5.6','ozon_client_id':'','company_name':'','telegram_chat_id':''}
...
def replace_products(mp,items):
    with sqlite3.connect(DB) as c:
        ...
        for p in items:c.execute('INSERT INTO products(marketplace,external_id,sku,name,price,stock,raw_json) VALUES(?,?,?,?,?,?,?)',(mp,p.get('external_id'),p.get('sku'),p.get('name'),p.get('price',0),p.get('stock',0),json.dumps(p,ensure_ascii=False)))
...
def save_project(name,photo,result):
    with sqlite3.connect(DB) as c:c.execute('INSERT INTO projects(name,photo,result_json) VALUES(?,?,?)',(name,photo,json.dumps(result,ensure_ascii=False)))
