...
from .autopilot import actions, save_card_version, queue_action
...
def _download_source_photo(url, nm_id):
    if not url:
        raise RuntimeError('У синхронизированной карточки WB не найдено исходное фото. Обновите каталог или создайте карточку через AI Фабрику вручную.')
    ...
    if not r.ok:
        raise RuntimeError(f'Не удалось загрузить исходное фото WB: HTTP {r.status_code}')
...
def _facts_from_raw(raw, nm_id):
    raw=raw or {}
    allowed={
        'nmID':raw.get('nmID') or nm_id,
        'vendorCode':raw.get('vendorCode'),
        'brand':raw.get('brand'),
        'title':raw.get('title'),
...
def _recently_processed(nm_id):
    ...
    for run in growth_runs(100):
        if str(run.get('entity_id'))==nm and run.get('status') in ('generated','queued','published'):
            return True
    for a in actions(300):
        if str(a.get('entity_id'))==nm and a.get('action_type') in ('publish_card','regenerate_card') and a.get('status') in ('proposed','approved','running'):
            return True
    return False
...
class AutonomousCardOptimizer:
    def __init__(self, wb, ai):
        self.wb=wb; self.ai=ai

    def choose_candidate(self, seo_rows, threshold=70):
        candidates=[]
        for row in seo_rows or []:
            if str(row.get('marketplace') or 'WB')!='WB':
                continue
            nm=str(row.get('nm') or row.get('external_id') or '')
            if not nm or float(row.get('score') or 100)>=float(threshold):
                continue
            if _recently_processed(nm):
                continue
            candidates.append(row)
        candidates.sort(key=lambda x: float(x.get('score') or 100))
        return candidates[0] if candidates else None

    def regenerate(self, row, slide_count=7, progress=None, is_cancelled=None):
        nm=str(row.get('nm') or row.get('external_id') or '')
        if not nm:
            raise RuntimeError('У карточки отсутствует nmID')
        raw=_raw_product(nm)
        if not raw:
            raise RuntimeError('Нет синхронизированного исходника WB для карточки '+nm)
        source=_download_source_photo(_photo_url(raw),nm)
        facts=_facts_from_raw(raw,nm)
        context=('Автоматическая оптимизация существующей карточки Wildberries. Используй только подтверждённые данные ниже. '
                 'Не меняй идентификатор товара, не выдумывай состав, размеры, сертификаты и свойства. '
                 'Создай сильнее текущей версии тексты, SEO, визуальную концепцию и инфографику.\n'+
        ...
        if pack.get('cancelled'):
            _record('WB',nm,row.get('score'), 'cancelled',source,notes='Остановлено пользователем')
            return {'cancelled':True,'nm':nm}
        card=pack.get('card') or {}; images=pack.get('images') or []
        version=save_card_version('WB',nm,card,images,'autopilot_generated')
        action_id=WBPublisher(self.wb).queue_publish(nm,card,images,
            f'CONTINUOUS AUTOPILOT: SEO score {row.get("score")}/100, AI полностью пересобрал карточку и визуал')
        run_id=_record('WB',nm,row.get('score'),'queued',source,version,action_id,
                       f'AI создал {len(images)} изображений; публикация #{action_id}')
