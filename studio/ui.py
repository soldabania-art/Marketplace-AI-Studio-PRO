...
STYLE='''QWidget{background:#0b1220;color:#eef2ff;font-family:Segoe UI;font-size:14px}QFrame#side{background:#111827}QPushButton{background:#1f2937;border:none;border-radius:8px;padding:10px;text-align:left}QPushButton:hover{background:#334155}QPushButton#primary{background:#2563eb;text-align:center;font-weight:700}QLineEdit,QTextEdit,QTableWidget,QComboBox,QDoubleSpinBox,QSpinBox{background:#111827;border:1px solid #334155;border-radius:7px;padding:7px}QLabel#title{font-size:28px;font-weight:800}QLabel#muted{color:#94a3b8}QGroupBox{border:1px solid #273449;border-radius:10px;margin-top:12px;padding:12px;font-weight:700}'''

class Main(QMainWindow):
    def __init__(self):
        super().__init__(); init_db(); self.cfg=load_settings(); self.photo=''; self.card=None
        self.setWindowTitle('Marketplace AI Studio PRO — Mega Edition 1.2'); self.resize(1500,920)
        ...
        version=QLabel('MEGA EDITION 1.2'); version.setStyleSheet('color:#60a5fa;padding:0 18px 14px 18px;font-weight:700'); sl.addWidget(version)
    ...
    def dashboard(self):
        ...
        row=QHBoxLayout()
        for a,b in [('Товаров в базе',str(len(products()))),('Wildberries','готов к синхронизации'),('Ozon','готов к синхронизации'),('AI','Card + Image Factory')]:
            g=QGroupBox(a); q=QVBoxLayout(g); q.addWidget(QLabel(b)); row.addWidget(g)
        v.addLayout(row)
        r=QHBoxLayout(); r.addWidget(self.primary('Синхронизировать WB',lambda:self.sync('WB'))); r.addWidget(self.primary('Синхронизировать Ozon',lambda:self.sync('Ozon'))); r.addWidget(self.primary('Создать карточку по фото',self.factory)); v.addLayout(r)
        note=QGroupBox('Что уже умеет Mega Edition 1.2'); nv=QVBoxLayout(note)
        nv.addWidget(QLabel('AI анализирует фото и факты о товаре, создаёт SEO-карточку и отдельно генерирует реальные вертикальные изображения инфографики через GPT-Image-2.'))
        nv.addWidget(QLabel('Отзывы: AI-ответ + проект аргументированной жалобы. Ключи хранятся через Windows Credential Manager.'))
        v.addWidget(note); v.addStretch(); self.showp(w)

    def factory(self):
        w,v=self.page('AI Фабрика','Фото + факты → полная карточка → реальные слайды инфографики')
        ...
        self.fact=QTextEdit(); self.fact.setPlaceholderText('Введите только реальные факты: материал, размеры, комплектацию, особенности, целевую аудиторию, ограничения...'); self.fact.setFixedHeight(140); v.addWidget(self.fact)
        settings=QHBoxLayout(); settings.addWidget(QLabel('Количество слайдов:')); self.slide_count=QSpinBox(); self.slide_count.setRange(1,10); self.slide_count.setValue(6); settings.addWidget(self.slide_count); settings.addStretch(); v.addLayout(settings)
        r=QHBoxLayout(); r.addWidget(self.primary('1. Сгенерировать полную карточку',self.generate_card)); r.addWidget(self.primary('2. Создать изображения инфографики',self.generate_images)); r.addWidget(self.primary('Экспорт XLSX',self.export_card)); r.addWidget(self.primary('Открыть папку изображений',self.open_generated)); v.addLayout(r)
        self.factory_status=QLabel('Готово к работе'); self.factory_status.setObjectName('muted'); v.addWidget(self.factory_status)
        self.cardout=QTextEdit(); self.cardout.setPlaceholderText('Здесь появится JSON полной карточки товара'); v.addWidget(self.cardout,1); self.showp(w)
    ...
    def generate_card(self):
        try:
            self.factory_status.setText('AI анализирует фото и создаёт карточку...')
            ...
            self.cardout.setPlainText(json.dumps(self.card,ensure_ascii=False,indent=2)); self.factory_status.setText('Карточка готова. Можно создавать изображения.')
        except Exception as e: self.factory_status.setText('Ошибка'); QMessageBox.critical(self,'AI',str(e))

    def generate_images(self):
        if not self.card:return QMessageBox.information(self,'Инфографика','Сначала нажмите «Сгенерировать полную карточку».')
        try:
            self.factory_status.setText('GPT-Image-2 создаёт изображения. Это может занять несколько минут...')
            ...
            QMessageBox.information(self,'Инфографика','Созданы файлы:\n'+'\n'.join(str(p) for p in paths))
        except Exception as e:self.factory_status.setText('Ошибка генерации изображений'); QMessageBox.critical(self,'Инфографика',str(e))
    ...
    def reviews_page(self):
        w,v=self.page('Отзывы','AI-ответы и подготовка обоснованной жалобы'); rating=QComboBox(); rating.addItems(['1','2','3','4','5']); txt=QTextEdit(); txt.setPlaceholderText('Текст отзыва'); out=QTextEdit(); v.addWidget(rating); v.addWidget(txt)
        r=QHBoxLayout()
        def reply():
            try:out.setPlainText(self.ai().review_reply(txt.toPlainText(),int(rating.currentText())))
            except Exception as e:QMessageBox.critical(self,'Отзывы',str(e))
        def complaint():
            try:out.setPlainText(self.ai().review_complaint(txt.toPlainText(),int(rating.currentText())))
            except Exception as e:QMessageBox.critical(self,'Жалоба',str(e))
        r.addWidget(self.primary('Сгенерировать ответ',reply)); r.addWidget(self.primary('Подготовить жалобу',complaint)); v.addLayout(r); v.addWidget(out)
        v.addWidget(QLabel('Автоматическая отправка жалобы будет подключаться только там, где это разрешено официальным API маркетплейса.')); self.showp(w)
    ...
    def director_page(self):
        ...
        def go():
            try:out.setPlainText(self.ai().director_plan(inp.toPlainText()))
