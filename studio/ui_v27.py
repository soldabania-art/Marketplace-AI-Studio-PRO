...
class Main(BaseMain):
    ...
    def _credit_text(self):
        ...
        if info.get('error'):
            return 'OpenAI: ' + str(info.get('error'))
        if info.get('spent_usd') is not None:
            spent = float(info.get('spent_usd') or 0)
            remaining = info.get('remaining_usd')
            if remaining is None:
                return f'OpenAI API расходы за месяц: ${spent:.2f} · задайте месячный бюджет, чтобы видеть остаток.'
            return f'OpenAI API: потрачено ${spent:.2f} · расчётный остаток бюджета ${float(remaining):.2f} из ${float(info.get("budget_usd") or 0):.2f}'
        if self.cfg.get('openai_admin_key'):
            return 'OpenAI API: нажмите «Обновить расходы / кредит».'
        return 'OpenAI API: для расходов нужен Admin API key. Обычный API key не показывает баланс организации.'
    ...
    def dashboard(self):
        ...
        credit = QGroupBox('OpenAI API — расходы и кредит')
        cv = QHBoxLayout(credit)
        self.credit_label = QLabel(self._credit_text()); self.credit_label.setWordWrap(True)
        cv.addWidget(self.credit_label, 1)
        cv.addWidget(self.primary('Обновить расходы / кредит', self.refresh_openai_credit))
        layout.addWidget(credit)

    def settings_page(self):
        super().settings_page()
        w = self.stack.currentWidget(); layout = w.layout()
        group = QGroupBox('OpenAI API — контроль расходов')
        form = QFormLayout(group)
        admin = QLineEdit(str(self.cfg.get('openai_admin_key','') or '')); admin.setEchoMode(QLineEdit.Password)
        budget = QLineEdit(str(self.cfg.get('openai_monthly_budget_usd',0) or 0))
        budget.setPlaceholderText('Например: 50')
        self.fields['openai_admin_key'] = admin
        self.fields['openai_monthly_budget_usd'] = budget
        form.addRow('OpenAI Admin API key:', admin)
        form.addRow('Месячный бюджет, USD:', budget)
        note = QLabel('Admin key нужен только для чтения официальных расходов Organization Costs API. OpenAI не предоставляет в публичном API отдельный точный prepaid credit balance, поэтому приложение показывает фактические расходы за месяц и расчётный остаток от указанного вами бюджета.')
        note.setWordWrap(True); form.addRow(note)
        form.addRow(self.primary('Обновить расходы сейчас', self.refresh_openai_credit))
    ...
    def save_cfg(self):
        if hasattr(self, 'fields') and 'openai_monthly_budget_usd' in self.fields:
            ...
            try:
                ...
                return QMessageBox.warning(self, 'Настройки', 'Месячный бюджет OpenAI должен быть числом, например 50 или 25.5.')
            self.fields['openai_monthly_budget_usd'].setText(str(value))
        return super().save_cfg()

    def refresh_openai_credit(self):
        if hasattr(self, 'fields'):
            admin_widget = self.fields.get('openai_admin_key')
            budget_widget = self.fields.get('openai_monthly_budget_usd')
            admin_key = admin_widget.text().strip() if admin_widget is not None else self.cfg.get('openai_admin_key','')
            raw_budget = budget_widget.text().strip().replace(',', '.') if budget_widget is not None else str(self.cfg.get('openai_monthly_budget_usd',0) or 0)
            try: budget = float(raw_budget or 0)
            except Exception: budget = 0.0
        else:
            admin_key = self.cfg.get('openai_admin_key','')
            try: budget = float(self.cfg.get('openai_monthly_budget_usd',0) or 0)
            except Exception: budget = 0.0
        def job(progress, is_cancelled):
            progress(20, 'OpenAI: загружаю расходы организации...')
            data = organization_costs(admin_key, budget)
            progress(100, 'OpenAI: расходы обновлены')
            return data
    ...
    def _worker_error(self, title, error):
        ...
        if title == 'OpenAI расходы':
            self.openai_credit_info = {'error': str(error)}
            if hasattr(self, 'credit_label'): self.credit_label.setText(self._credit_text())
    ...
    def products_page(self):
        ...
        top = QHBoxLayout(); top.addWidget(self.primary('Обновить WB', self.sync_wb_full)); top.addWidget(self.primary('Обновить Ozon', self.sync_all_ozon)); top.addStretch(); v.addLayout(top)
        data = [dict(r) for r in products()]
        table = QTableWidget(); table.setColumnCount(6); table.setHorizontalHeaderLabels(['MP','ID','SKU','Название','Цена','Остаток']); table.setRowCount(len(data))
        table.setSelectionBehavior(QAbstractItemView.SelectRows); table.setSelectionMode(QAbstractItemView.SingleSelection); table.setEditTriggers(QAbstractItemView.NoEditTriggers); table.setCursor(Qt.PointingHandCursor)
        ...
        v.addWidget(table,1)
        hint=QLabel('Двойной клик по товару → открыть действия. Для WB можно сразу перейти в AI Фабрику с выбранным nmID.'); hint.setWordWrap(True); v.addWidget(hint); self.showp(w)

    def _product_actions(self, product):
        mp=str(product.get('marketplace') or ''); nm=str(product.get('external_id') or ''); name=str(product.get('name') or product.get('sku') or nm)
        ...
        ai_btn=box.addButton('Открыть в AI Фабрике', QMessageBox.ActionRole); refresh_btn=box.addButton('Обновить каталог', QMessageBox.ActionRole); box.addButton('Закрыть', QMessageBox.RejectRole); box.exec()
        clicked=box.clickedButton()
        if clicked is ai_btn:
            self.factory()
            for attr in ('factory_nm','product_nm','nm_input','nm_id_input'):
                widget=getattr(self,attr,None)
                if widget is not None and hasattr(widget,'setText'): widget.setText(nm); break
        elif clicked is refresh_btn:
            self.sync_wb_full() if mp == 'WB' else self.sync_all_ozon()

    def refresh_ads_only(self):
        def job(progress, is_cancelled):
            progress(15, 'WB реклама: загружаю список кампаний...'); wb = self.wb(); groups = wb.campaign_groups()
            if is_cancelled(): return {'cancelled': True}
            progress(55, 'WB реклама: загружаю статистику с контролем лимита API...'); ads = self.business().ad_stats(groups, 30); progress(100, 'Рекламная статистика обновлена'); return {'ads': ads}
        return self._start_worker('Реклама WB', job, self._ads_ready)

    def _ads_ready(self, result):
        if result.get('cancelled'): return
        self.ads_live = result.get('ads') or {}
        if hasattr(self, 'functional_status'): self.functional_status.setText(f"Реклама WB: расход {self.ads_live.get('spend',0):,.2f} ₽, ДРР {float(self.ads_live.get('drr') or 0):.2f}%")
        self.ads_page()

    def load_reviews(self):
        answered = self.rev_filter.currentIndex() == 1 if hasattr(self, 'rev_filter') else False
        def job(progress, is_cancelled):
            progress(20, 'WB отзывы: загрузка...'); rows = self.wb().reviews(is_answered=answered, take=100); progress(100, f'Отзывы загружены: {len(rows)}'); return rows
    ...
    def refresh_wb_business_data(self):
        def job(progress, is_cancelled):
            ...
            s_by_nm = sales_by_nm(sales); sku = sku_profitability(items, fsum.get('by_nm', {}) if isinstance(fsum, dict) else {}, s_by_nm, ads.get('spend',0) if isinstance(ads,dict) else 0, load_cogs())
            ...
            progress(100, 'WB: полный цикл данных готов'); return {'items': items, 'live': live, 'finance': fsum, 'ads': ads, 'sku': sku}
    ...
    def test(self, mp):
        def job(progress, is_cancelled):
            progress(30, f'Проверяю {mp}...'); msg = self.wb().test() if mp == 'WB' else self.ozon().test(); progress(100, msg); return msg
