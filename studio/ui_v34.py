...
class Main(BaseMain):
    """3.4 stability audit: friendly errors, cancellation-safe WB jobs and exact ad attribution."""
    ...
    def sync_wb_full(self):
        def job(progress,is_cancelled):
            ...
            progress(55,f'WB: карточек {len(items)}, цены и остатки...'); items=self.wb().hydrate_products(items)
            if is_cancelled(): return {'cancelled':True,'items':[]}
            progress(95,'WB: сохраняю каталог...'); return {'cancelled':False,'items':items}
    ...
    def _wb_sync_ready(self,result):
        if (result or {}).get('cancelled'):
            self.statusBar().showMessage('Синхронизация WB остановлена'); return
        items=(result or {}).get('items') or []; replace_products('WB',items)
        if hasattr(self,'functional_status'): self.functional_status.setText(f'WB каталог обновлён: {len(items)} товаров')
        QMessageBox.information(self,'Wildberries',f'Каталог обновлён: {len(items)} товаров.'); self.products_page()

    def refresh_ads_only(self):
        def job(progress,is_cancelled):
            progress(15,'WB реклама: список кампаний...'); groups=self.wb().campaign_groups()
            if is_cancelled(): return {'cancelled':True}
            progress(55,'WB реклама: статистика...'); ads=self.business().ad_stats(groups,30)
            if is_cancelled(): return {'cancelled':True}
            ads['by_nm']=WBAdManager(self.wb()).sku_totals(ads.get('rows') or [])
            progress(100,'Реклама обновлена'); return {'cancelled':False,'ads':ads}
    ...
    def _ads_ready_v34(self,result):
        ...
        self.ads_live=(result or {}).get('ads') or {}; self.ads_page()

    def load_reviews(self):
        answered=self.rev_filter.currentIndex()==1 if hasattr(self,'rev_filter') else False
        def job(progress,is_cancelled):
            progress(20,'WB отзывы: загрузка...'); rows=self.wb().reviews(is_answered=answered,take=100)
            if is_cancelled(): return {'cancelled':True,'rows':[]}
            progress(100,f'Отзывы: {len(rows)}'); return {'cancelled':False,'rows':rows}
    ...
    def refresh_wb_business_data(self):
        def job(progress,is_cancelled):
            ...
            if is_cancelled(): return {'cancelled':True}
            try:
                groups=wb.campaign_groups(); ads=biz.ad_stats(groups,30); ads['by_nm']=WBAdManager(wb).sku_totals(ads.get('rows') or [])
            except Exception as e:groups={'error':str(e)}; ads={'error':str(e),'by_nm':{}}
            if is_cancelled(): return {'cancelled':True}
            try:rating=wb.seller_rating()
            except Exception as e:rating={'error':str(e)}
            sku=sku_profitability(items,fsum.get('by_nm',{}),sales_by_nm(sales),ads.get('spend',0) if isinstance(ads,dict) else 0,load_cogs(),ads.get('by_nm',{}) if isinstance(ads,dict) else {})
            ...
            progress(100,'WB: готово'); return {'cancelled':False,'items':items,'live':live,'finance':fsum,'ads':ads,'sku':sku}
        return self._start_worker('WB бизнес-данные',job,self._business_ready_v34)

    def _business_ready_v34(self,result):
        if (result or {}).get('cancelled'):
            self.statusBar().showMessage('Обновление WB остановлено'); return
        replace_products('WB',(result or {}).get('items') or [])
        self.live=(result or {}).get('live') or {}; self.finance_live=(result or {}).get('finance') or {}; self.ads_live=(result or {}).get('ads') or {}; self.sku_live=(result or {}).get('sku') or []
        self.alerts_live=build_alerts([dict(r) for r in products()],self.live,self.finance_live,self.ads_live,load_cogs()); self.dashboard()
