...
- #1–#8 и дочерние #37–#39 закрыты. D01/#46 принят главным архитектором как отдельный визуальный эталон без изменения backend-контрактов; интеграция и публикация ещё не подтверждены. T09–T22 и #44 остаются в очереди. D02/#48 реализован и проверен локально в отдельной ветке; remote CI, browser verification, визуальная приёмка, публикация и интеграция не выполнялись.
...
[D02/#48](https://github.com/soldabania-art/Marketplace-AI-Studio-PRO/issues/48) реализован локально в `codex/d02-public-entry` от базового D01 SHA `a67fa6b1702b5e0e7b418bbf43293cb9431174a6`. Зафиксированный функциональный head D02 — `32b6cc6fd467c4689f821a67409bb5ec098d46d1`; последующие изменения до разрешённого preview допускаются только документные и не меняют runtime. Публичная главная и выбор перенесены в принятое направление B, auth URL сохранены, query регистрации нормализуется по allowlist, а выбранные площадка, модули и количество магазинов сохраняются при возврате из регистрации к `/#bundle`. Выбор остаётся только запросом, будущая площадка — интересом к запланированной интеграции. Локально зелёные 25 frontend-тестов, Next.js production build и backend suite на изолированной test DB: 244 passed, 22 skipped.
...
**D02 остаётся не принятым и не опубликованным:** Cloud Browser не имеет разрешённого доступа к локальному preview; HTTP smoke и статический анализ не заменяют browser verification. Не выполнены настоящие проверки 1440/1280/390, обычного и некорректного query, будущей площадки, cookie «Только обязательные» и сохранённого consent, клавиатуры, reduced motion, длинных названий, console/runtime errors и overflow. Отдельно проверить маршрут выбор → регистрация → назад к `/#bundle`: площадка, модули и количество магазинов должны восстановиться в видимых элементах интерфейса, а не только сохраниться в URL. Remote CI и Vercel точного D02 head не запускались.

После снятия лимита выполнить цепочку без пропуска этапов: D01 deployment → интеграция D01 → проверка итогового `main` → перенос D02 на этот `main` с контролем diff → отдельная опубликованная ветка и **draft PR** → настоящая browser-проверка разрешённого preview → полный exact-head CI и Vercel → перевод PR в ready for независимую приёмку. Итоговые доказательства должны включать точный SHA, реальные снимки и запись маршрута. D02 не сливать.
