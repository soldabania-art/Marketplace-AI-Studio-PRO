...
| AI Support Agent | Incident-safe intake plus source-gated product-help retrieval | Curated approved TROVENDI documentation, answer quality evaluation and feedback loop |
...
5. Incident-safe AI Support Agent: deterministic escalation, idempotent ticket, redacted evidence and authenticated emergency STOP entry point before product-help RAG. **Implemented P1A foundation:** a store-scoped incident intake classifies security, financial-loss, unexpected-write and legal/billing signals deterministically before any model call; forced escalation cannot be downgraded, duplicate reports produce one ticket, and the attached bundle contains only bounded health/control/audit indexes (no secrets, customer data, raw audit payloads or finance rows). STOP remains a separate owner/admin confirmation in AI Director. **Implemented P1B foundation:** product-help retrieves only platform-admin-reviewed, current, versioned documents; every answer displays its document/version/review date, and an unmatched question explicitly reports missing approved knowledge instead of improvising. Drafts, expired versions and risk signals are never used for an answer. Curated documentation, evaluation, feedback and the human support queue remain delivery gates.
