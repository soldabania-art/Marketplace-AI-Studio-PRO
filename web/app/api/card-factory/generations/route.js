...
export async function GET(request){
  ...
  try{const {response,payload}=await backendRequest(`/api/v1/card-factory/generations?${query}`,{headers:{Authorization:`Bearer ${token}`}});if(!response.ok)return NextResponse.json({error:payload?.detail||'Не удалось загрузить историю AI'},{status:response.status});return NextResponse.json(payload)}catch{return NextResponse.json({error:'История AI временно недоступна.'},{status:503})}
}
