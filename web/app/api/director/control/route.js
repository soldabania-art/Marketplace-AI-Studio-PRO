...
export async function PATCH(request){
  ...
  try{
    ...
    const {response,payload}=await backendRequest('/api/v1/director/control',{
      method:'PATCH',headers:{Authorization:`Bearer ${session}`},body:JSON.stringify(body)})
    if(!response.ok)return NextResponse.json({error:payload?.detail||'Не удалось изменить режим'},{status:response.status})
