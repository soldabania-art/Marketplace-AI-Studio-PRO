...
export async function GET(request){
  ...
  if(!token) return NextResponse.json({error:'Войдите в аккаунт и подключите магазин, чтобы искать свободные склады.'},{status:401})
  ...
  }catch(error){return NextResponse.json({error:'Сервис поиска складов пока недоступен.'},{status:503})}
}
