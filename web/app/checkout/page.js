...
function CheckoutForm(){
 ...
 return <main className="checkoutPage"><div className="checkoutWrap"><div className="workHead"><Link href="/pricing" className="ghostBtn"><ArrowLeft size={16}/> Тарифы</Link><span className="checkoutSecure"><LockKeyhole size={15}/> Безопасная оплата</span></div><section className="checkoutCard"><span className="eyebrow">ОФОРМЛЕНИЕ ПОДПИСКИ</span><h1>{plan.name}</h1><div className="checkoutPrice">{plan.price}<small> / {plan.period}</small></div><div className="checkoutSummary"><div><span>Тариф</span><strong>{plan.name}</strong></div><div><span>Списание</span><strong>ежемесячно</strong></div><div><span>К оплате сейчас</span><strong>{plan.price}</strong></div></div><label className="checkoutConsent"><input type="checkbox" checked={accepted} onChange={e=>setAccepted(e.target.checked)}/> Я принимаю условия подписки, оферту и правила автоматического продления.</label><button className="payBtn" disabled={!accepted} onClick={pay}><CreditCard size={18}/> Перейти к оплате</button>{message&&<div className="checkoutMessage">{message}</div>}<p className="checkoutNote">Данные карты Marketplace AI Studio не хранит. Оплата должна проходить на стороне подключённого платёжного провайдера.</p></section></div></main>
}
