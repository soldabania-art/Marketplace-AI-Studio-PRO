...
function CheckoutForm(){
 ...
 return <main className="checkoutPage"><div className="checkoutWrap"><div className="workHead"><Link href="/pricing" className="ghostBtn"><ArrowLeft size={16}/> Тарифы</Link><span className="checkoutSecure"><LockKeyhole size={15}/> Безопасная оплата</span></div><section className="checkoutCard"><span className="eyebrow">ОФОРМЛЕНИЕ ПОДПИСКИ</span><h1>{plan.name}</h1><div className="checkoutPrice">{plan.price}<small> / {plan.period}</small></div><div className="checkoutSummary"><div><span>Тариф</span><strong>{plan.name}</strong></div><div><span>Списание</span><strong>ежемесячно</strong></div><div><span>К оплате сейчас</span><strong>{plan.price}</strong></div><div><span>После оплаты</span><strong>настройка MFA</strong></div></div><label className="checkoutConsent"><input type="checkbox" checked={accepted} onChange={e=>setAccepted(e.target.checked)}/> Я принимаю условия подписки, оферту и правила автоматического продления.</label><button className="payBtn" disabled={!accepted||busy} onClick={pay}><CreditCard size={18}/> {busy?'Открываем защищённую оплату…':'Перейти к оплате'}</button>{message&&<div className="checkoutMessage" role="alert">{message}</div>}<p className="checkoutNote">TROVENDI не хранит данные банковской карты. Платные права включаются только после подтверждения платежа сервером провайдера; перед подключением магазина потребуется MFA.</p></section></div></main>
}
