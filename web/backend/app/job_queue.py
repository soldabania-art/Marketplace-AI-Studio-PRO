...
def enqueue(db:Session,*,job_type:str,idempotency_key:str,payload:dict|None=None,workspace_id:str|None=None,store_id:str|None=None,priority:int=100,max_attempts:int=5,available_at:datetime|None=None)->BackgroundJob:
    ...
    job=BackgroundJob(job_type=job_type,idempotency_key=idempotency_key,payload=payload or {},workspace_id=workspace_id,store_id=store_id,priority=priority,max_attempts=max(1,max_attempts),available_at=available_at or utcnow())
    db.add(job)
    try:
...
def _claim_one(db:Session,worker_id:str,lease_seconds:int)->BackgroundJob|None:
    now=utcnow(); stale=now-timedelta(seconds=lease_seconds)
    ...
    ).order_by(BackgroundJob.priority.asc(),BackgroundJob.available_at.asc(),BackgroundJob.created_at.asc()))
