...
class WatchBody(BaseModel): marketplace:str='wildberries'; warehouse_filter:str=''; free_only:bool=False; enabled:bool=True
...
def save_watch(body:WatchBody,user:User=Depends(get_current_user),db:Session=Depends(get_db)):
 ...
 row.warehouse_filter=body.warehouse_filter[:500]; row.free_only=body.free_only; row.enabled=body.enabled; db.commit(); return {'ok':True,'enabled':row.enabled,'watch_id':row.id,'scope':'all_warehouses' if not row.warehouse_filter else 'filtered','coefficients':[0] if row.free_only else [0,1]}
