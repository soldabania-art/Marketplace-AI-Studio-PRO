...
from .models import PlatformStaffRoleName, User, UserMfa, UserSession
from .platform_access import get_effective_platform_role
...
def is_platform_admin(db: Session, user: User) -> bool:
    return get_effective_platform_role(db, user.id) == PlatformStaffRoleName.owner
...
def _require_platform_roles(
    ...
    db: Session,
    allowed: set[PlatformStaffRoleName],
) -> User:
    current_user = db.scalar(select(User).where(
        User.id == current_session.user_id, User.is_active.is_(True)
    ).execution_options(populate_existing=True))
    if current_user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="User not found")
    if get_effective_platform_role(db, current_user.id) not in allowed:
    ...
    if mfa is None or not mfa.enabled or current_session.mfa_verified_at is None:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Platform staff MFA is required")
    return current_user


def require_platform_admin(current_session: UserSession = Depends(get_current_session), db: Session = Depends(get_db)) -> User:
    return _require_platform_roles(current_session, db, {PlatformStaffRoleName.owner})


def require_platform_summary(current_session: UserSession = Depends(get_current_session), db: Session = Depends(get_db)) -> User:
    return _require_platform_roles(
        current_session,
        db,
        {PlatformStaffRoleName.owner, PlatformStaffRoleName.project_manager},
    )
