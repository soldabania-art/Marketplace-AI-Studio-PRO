...
from . import account_mail  # noqa: F401 - registers account email handler
from .fbo_monitor import monitor_forever
from .account_mail import account_mail_cleanup_forever
...
async def _run()->None:
    ...
    tasks=[asyncio.create_task(monitor_forever(stop_event),name='fbo-scheduler'),asyncio.create_task(sync_scheduler_forever(stop_event),name='sync-scheduler'),asyncio.create_task(account_mail_cleanup_forever(stop_event),name='account-mail-cleanup'),asyncio.create_task(job_worker_forever(stop_event),name='job-queue')]
    try:
