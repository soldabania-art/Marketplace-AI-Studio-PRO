...
export default function DailyDirectorWorkspace(){
  ...
  return <main className="workPage directorPage">
    ...
    {data&&<>
      {data.control.stopped&&<div className="sectionNotice" role="alert"><PauseCircle size={18}/><b>Emergency STOP действует для этого магазина.</b> Все новые публикации карточек и фото в WB заблокированы; чтение и диагностика доступны.</div>}
      ...
      <section className="workPanel directorSources"><div className="directorSectionHead"><div><span className="eyebrow">ДОКАЗАТЕЛЬНАЯ БАЗА</span><h2>Источники решения</h2></div><small>Актуальность: по политике каждого источника</small></div><div className="directorSourceGrid">{data.sources.map(source=><div className={source.state} key={source.name}>{source.state==='live'?<CheckCircle2 size={17}/>:<Clock3 size={17}/>}<span>{sourceNames[source.name]||source.name}</span><b>{stateNames[source.state]}</b></div>)}</div></section>
      ...
      <div className="directorGuard"><ShieldCheck size={19}/><div><b>Контур безопасности включён</b><span>{data.automation.note} Подтверждение сохраняется отдельно от исполнения; STOP повторно проверяется прямо перед каждой новой записью. Уже принятый провайдером запрос отменить этим переключателем нельзя.</span></div></div>
    </>}
  </main>
}
