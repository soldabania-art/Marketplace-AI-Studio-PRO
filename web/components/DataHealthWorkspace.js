...
const statusMeta={
  ...
  incomplete:{label:'Период неполный',icon:AlertTriangle,tone:'stale'},
...
export default function DataHealthWorkspace(){
  ...
  return <main className="healthPage"><header className="healthTop"><Link href="/" className="ghostBtn"><ArrowLeft size={16}/> На главную</Link><Link href="/account" className="ghostBtn">Подключения</Link></header><section className="healthContent">
    ...
    {data.active_incidents?.length>0&&<section className="incidentPanel"><div className="incidentTitle"><ShieldAlert size={19}/><div><strong>Активные сигналы AI Director</strong><span>Повторные уведомления объединяются, а сигнал закрывается только после восстановления источника.</span></div></div>{data.active_incidents.map(item=><article key={item.id}><b>{item.severity==='critical'?'Критично':'Предупреждение'}</b><p>{item.message}</p><time>{new Date(item.opened_at).toLocaleString('ru-RU')}</time></article>)}</section>}
    <section className="sourceGrid">{data.sources.map(source=>{const meta=statusMeta[source.status]||statusMeta.missing;const Icon=meta.icon;return <article className={`sourceCard ${meta.tone}`} key={source.key}><div className="sourceHead"><div className="sourceIcon"><Icon size={19}/></div><span>{meta.label}</span></div><h2>{source.label}</h2><p>{age(source.age_seconds)}</p><div className="sourceMeta"><span>Допуск: {Math.floor(source.warn_after_seconds/60)} мин</span><span>{source.record_count===null||source.record_count===undefined?'Без счётчика':`${source.record_count} записей`}</span></div><div className="usedBy">Используют: {source.required_for.join(' · ')}</div>{source.issue&&<div className="sourceIssue">{source.issue}</div>}{source.job&&<small>Задача: {source.job.status} · попыток {source.job.attempts}{source.refresh_in_progress?' · обновление уже запущено':''}</small>}</article>})}</section></>}
  </section></main>
}
