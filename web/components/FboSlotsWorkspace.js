...
export default function FboSlotsWorkspace(){
  ...
  useEffect(()=>{
    ...
    const selected=typeof window!=='undefined'?localStorage.getItem('mai_store_id')||'':''
    setStoreId(selected)
    if(selected){
      fetch('/api/stores',{cache:'no-store'}).then(r=>r.json()).then(p=>{
        const store=p.stores?.find(x=>x.id===selected)
        if(store)setStoreName(store.name)
      }).catch(()=>{})
      fetch(`/api/marketplace/fbo-watch?store_id=${encodeURIComponent(selected)}`,{cache:'no-store'}).then(r=>r.json()).then(p=>{
        if(typeof p.enabled==='boolean')setWatching(p.enabled)
      }).catch(()=>{})
    }
  },[])

  async function search(){
    if(!storeId){setMessage('Сначала выберите магазин в разделе «Подключения».');return false}
  ...
  async function toggleWatch(){
    if(!storeId){setMessage('Сначала выберите магазин в разделе «Подключения».');return}
    setMessage('')
    try{
      if(watching){
        ...
        setWatching(false); setMessage('Серверный мониторинг остановлен.'); return
      }
      ...
      setWatching(true)
      setMessage(`Мониторинг активен 24/7 для магазина ${storeName||'WB'}: все склады, коэффициенты ${payload.coefficients?.join(' и ')||'0 и 1'}.`)
      await search()
  ...
  return <main className="workPage">
    ...
    <section className="workHero"><span className="eyebrow">FBO / FBW ПОСТАВКИ</span><h1>Свободные склады для приёмки</h1><p>Реальный поиск по выбранному магазину Wildberries. Серверный мониторинг работает независимо от открытой страницы.</p></section>
    <div className="sectionNotice">Магазин: {storeName||'не выбран'} · PUSH: {permission==='granted'?(pushReady?'подключён':'разрешён'):(permission==='denied'?'заблокирован браузером':'нужно разрешение')} · Мониторинг: {watching?'активен':'выключен'} · Коэффициенты: {freeOnly?'0':'0 и 1'}</div>
