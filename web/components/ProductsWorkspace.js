...
import { ArrowLeft, Search, WandSparkles } from 'lucide-react'
...
export default function ProductsWorkspace(){
 ...
 <section className="workHero"><span className="eyebrow">КАТАЛОГ</span><h1>Товары</h1><p>Рабочий центр карточек WB и Ozon. Сейчас показаны безопасные демо-данные до подключения магазина.</p></section>
 <div className="workGrid"><section className="workPanel"><div className="toolbar"><div className="searchBox"><Search size={17}/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Поиск по товару или SKU"/></div><button onClick={()=>setNotice('Импорт запустится после подключения WB/Ozon в аккаунте.')}>Импортировать</button></div>
